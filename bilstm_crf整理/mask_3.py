...
if type_dataset == "NCBI":
    new_type_path = new_type_path+'/NCBI-autoNER/'
...
train_path = new_type_path +"train-new1.json"
dev_path = new_type_path +"dev.json"
test_path = new_type_path +"test.json"

if type_dataset == 'BC5-autoNER':
    train_path = new_type_path + "train-mask.json"
    dev_path = new_type_path+'dev.json'
    test_path = new_type_path+'test.json'
if type_dataset == 'BC5_ours':
    train_path = new_type_path + "train-mask.json"
...
def evaluating(model, datas, best_F, dataset="Train"):
    ...
    output_txt_path = path + '/' + type_dataset + '_mask_pred.txt'
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    with open(path+"/outcome_mask.txt","a+",encoding="UTF-8") as file:
        file.write("\n")
        ...
        for i, index in enumerate(np.random.permutation(len(train_dataset))):
            ...
            if count%6000 == 0:
                ...
                if new_dev_F > best_dev__F:
                    ...
                    torch.save(model.state_dict(), path+"/"+type_dataset+"_mask.pth")
...
path = 'data_' + type_dataset
model.load_state_dict(torch.load(path+"/"+type_dataset+"_mask.pth"))
model.train(False)
