...
new_type_path = '/media/sdd1/luosx/HuaiyuanYing/DecodeNER/all_other_experiments/datasets/type'
type_dataset = input("输入要得到的数据集:(NCBI,NCBI_ours,BC5-autoNER,BC5_ours)+(parse?)")
...
if type_dataset == 'BC5-autoNER':
    train_path = new_type_path + "/train-score-BC5auto-newtag1type.json"
if type_dataset == 'BC5_ours':
    train_path = new_type_path + "/train-score-BC5our-newtag1type.json"
if type_dataset == 'NCBI_ours':
    train_path = new_type_path + "/train-score-NCBIour-newtag1type.json"
if type_dataset == "NCBI":
    train_path = new_type_path + "/train-score-NCBIauto-newtag1type.json"
if type_dataset == 'BC5-autoNER_parse':
    train_path = new_type_path + "/train-score-BC5autoparse-newtag1type.json"
if type_dataset == 'BC5_ours_parse':
    train_path = new_type_path + "/train-score-BC5ourparse-newtag1type.json"
if type_dataset == 'NCBI_ours_parse':
    train_path = new_type_path + "/train-score-NCBIourparse-newtag1type.json"
if type_dataset == "NCBI_parse":
    train_path = new_type_path + "/train-score-NCBIautoparse-newtag1type.json"

dev_test_path = '/media/sdd1/luosx/HuaiyuanYing/DecodeNER/all_other_experiments/datasets'
if type_dataset == "NCBI" or type_dataset == "NCBI_parse":
    new_type_path = dev_test_path+'/NCBI-autoNER/'
if type_dataset == 'BC5-autoNER' or type_dataset == "BC5-autoNER_parse":
    new_type_path = dev_test_path + '/BC5-autoNER/'
if type_dataset == 'BC5_ours' or type_dataset == 'BC5_ours_parse':
    new_type_path = dev_test_path+'/BC5-ours/'
if type_dataset == 'NCBI_ours' or type_dataset == 'NCBI_ours_parse':
    new_type_path = dev_test_path + '/NCBI-ours/'
...
def evaluating(model, datas, best_F, dataset="Train"):
    ...
    output_txt_path = path + '/' + type_dataset + '_new_type_pred.txt'
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    with open(path+"/outcome_new_type.txt","a+",encoding="UTF-8") as file:
        file.write("\n")
...
def change_ner(y):
    new_y = []
    singer_y = 'O'
    if type_dataset == 'NCBI' or type_dataset == 'NCBI_ours' or type_dataset == 'NCBI_parse' or type_dataset == 'NCBI_ours_parse':
        for inst in y:
            if type(inst)!= int:
                inst = inst.item()
        ...
        return new_y
    if type_dataset == 'BC5-autoNER' or type_dataset == 'BC5_ours' or type_dataset == 'BC5-autoNER_parse' or type_dataset == 'BC5_ours_parse':
        for inst in y:
            if type(inst)!= int:
                inst = inst.item()
            inst = idx_to_tag[inst]
        ...
        for i, index in enumerate(np.random.permutation(len(train_dataset))):
            ...
            if count%4000 == 0:
                ...
                if new_dev_F > best_dev__F:
                    ...
                    torch.save(model.state_dict(), path+"/"+type_dataset+"_new_type.pth")
...
path = 'data_' + type_dataset
model.load_state_dict(torch.load(path+"/"+type_dataset+"_new_type.pth"))
model.train(False)
