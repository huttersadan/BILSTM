...
dirs = 'data_st21/'
new_type_path = '/media/sdd1/luosx/NER/our_method_datasets'
type_dataset = input("输入要得到的数据集(NCBI,medmentions,st21,bc5)")
...
if type_dataset == 'medmentions':
    new_type_path = new_type_path+'/medmention_extratraining/'

if type_dataset == 'st21':
    new_type_path = new_type_path+'/medmention_st21/'
if type_dataset == 'bc5':
    new_type_path = new_type_path+'/bc5/'
def input_data(path):
    insts = []
    seqs = []
    single_seq = []
...
train_path = new_type_path +"train_parse.json"
if type_dataset == 'NCBI':
    train_path = new_type_path + "train-parse.json"
...
def evaluating(model, datas, best_F, dataset="Train"):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    output_txt_path = path + '/' + type_dataset + '_parse_pred.txt'
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    with open(path+"/outcome_parse.txt","a+",encoding="UTF-8") as file:
        file.write("\n")
...
model.load_state_dict(torch.load(path+"/"+type_dataset+"_parse.pth"))
model.train(False)
