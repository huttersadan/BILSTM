...
def evaluating(model, datas, best_F, dataset="Train"):
    ...
    with open("data_st21/outcome_original.txt", "a+", encoding="UTF-8") as file:
        file.write("\n")
        ...
        for i, index in enumerate(np.random.permutation(len(train_dataset))):
            ...
            if count % (eval_every) == 0 and count > (eval_every * 20) or \
                ...
                if new_dev_F >best_dev__F:
                    torch.save(model.state_dict(), "data_st21/st21_original.pth")
...
model.load_state_dict(torch.load("data_st21/st21_original.pth"))
model.train(False)
