...
# train_path = "data_BC5_A/train.json"
# dev_path = "data_BC5_A/dev.json"
# test_path = "data_BC5_A/test.json"
# train_path = "data_debug/train_debug.json"
# dev_path = "data_debug/dev_devbug.json"
# test_path = "data_debug/test_debug.json"
train_path = total_path+'/train.json'
dev_path = total_path+'/dev.json'
test_path = total_path+'/test.json'
# train_path = "data_medmentions/test.json"
# dev_path = "data_medmentions/test.json"
# test_path = "data_medmentions/test.json"
train_json = input_data_json(train_path)
##dev_json = input_data_json(dev_path)
test_json = input_data_json(test_path)

total = train_json+test_json
random.shuffle(total)
length = len(total)
del train_json
del test_json
train_json = total[0:(17*length)//20]
test_json = total[(17*length)//20:(18*length)//20]
dev_json = total[(18*length)//20:length-1]
...
def viterbi_algo(self, feats):
    ...
    return path_score, best_path, collect_pred
    ...
    def __init__(self, vocab_size, tag_to_idx, embedding_dim, hidden_size,
        ...
        if char_embedding_dim is not None:
            ...
            self.char_cnn3 = nn.Conv2d(in_channels=1, out_channels=self.out_channels,
                                       kernel_size=(3, char_embedding_dim), padding=(1, 0))
...
def evaluating(model, datas, best_F, dataset="Train"):
    ...
    with open("data_medmentions/outcome_original.txt", "a+", encoding="UTF-8") as file:
        file.write("\n")
        ...
        for i, index in enumerate(np.random.permutation(len(train_dataset))):
            ...
            if True:
                ...
                chars2_mask = np.zeros((len(chars2_length), char_maxl),
                                       dtype='int')  # len(seq)*num_of_char_for_a_single_word
                for i, c in enumerate(chars2):
                    chars2_mask[i, :chars2_length[i]] = c
            ...
            if count%(len(train_dataset)//50) == 0:
            # if count%1000 == 0:
                model.train(False)
                ...
                if new_dev_F > best_dev__F:
                    ...
                    torch.save(model.state_dict(),'data_medmentions/medmentions_ex_original.pth')
...
model.load_state_dict(torch.load('data_medmentions/medmentions_ex_original.pth'))
