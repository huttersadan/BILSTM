...
total_path = "/media/sdd1/luosx/NER/our_method_datasets/medmention_st21/"
...
def evaluating(model, datas, best_F, dataset="Train"):
    ...
    with open("data_st21/outcome_parse.txt","a+",encoding="UTF-8") as file:
        file.write("\n")
...
if True:
    ...
    for epoch in trange(1, num_of_epoch):
        ...
        if new_dev_F  > best_dev___F:
            ...
            torch.save(model.state_dict(), dirs+"st21_parse.pth")
        model.train(True)

            # Performing decay on the learning rate

    model = torch.load(dirs+"st21_parse.pth")
